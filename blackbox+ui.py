...
def get_user_input():
    ...
    return username, choice, duration, num_photos, random_captures
...
# Get user input
username, choice, duration, num_photos, random_captures = get_user_input()
...
# Save to GIF if selected
if choice == 'gif':
    frames[0].save(f"{username}_burst_combined.gif", save_all=True, append_images=frames[1:], duration=100, loop=0)
    print(f"GIF saved as {username}_burst_combined.gif")
else:
    # Save the last frame as a single photo
    frames[-1].save(f"{username}_single_photo.png")
    print(f"Single photo saved as {username}_single_photo.png")
